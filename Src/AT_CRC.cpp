#include "AT_CRC.h"
#include "exeptions.h"
#include <stdio.h>

using namespace CRC_USR;

uint16_t crccheck = 0;

namespace CRC_USR
{
  
  cCRC CRC_global;
      /* Table of CRC values for high-order byte */
   const unsigned char auchCRCHi[] = {
   
   } ;

   /* Table of CRC values for low-order byte */
   const unsigned char auchCRCLo[] = {
   
   };
  
  uint16_t cCRC::calcByte(uint8_t Sbyte, uint16_t CRCbuf)
  {
    unsigned char uIndex ;
    uIndex = (LO2BYTE(CRCbuf)) ^ Sbyte ;
    *((unsigned char *)&CRCbuf+1) = (unsigned char)(LOBYTE(CRCbuf)) ^ auchCRCHi[uIndex] ;
    *((unsigned char *)&CRCbuf) = auchCRCLo[uIndex] ;
    return CRCbuf;
  }
  
  uint16_t cCRC::calc(uint8_t volatile * pData, uint32_t len, tagCRC_type type)
  {
    if(pData == NULL || len == 0)
    {
      exception(__FILE__,__FUNCTION__,__LINE__,"invalid buffer");
      return 0;
    }
    uint16_t CRCbuf = 0xffff;
    int i = 0;
    for(; i<len; i++)
    {
      CRCbuf = calcByte(pData[i], CRCbuf);
    }
    if(type == AT_CRC)
    {
        pData[i++] = LOBYTE(CRCbuf);
        pData[i++] = LO2BYTE(CRCbuf);
    }
    else
    {
        pData[i++] = LO2BYTE(CRCbuf);
        pData[i++] = LOBYTE(CRCbuf);
    }
    return i;
  }
  
  bool cCRC::check(const uint8_t * pData, uint32_t len, tagCRC_type type)
  {
    if(pData == NULL || len == 0)
    {
      exception(__FILE__,__FUNCTION__,__LINE__,"invalid buffer");
      return 0;
    }
    uint8_t bt1,bt2;
    uint16_t CRCbuf = 0xffff;
    if(len < 2)
      return false;
    for(int i = 0; i < len-2; i++)
    {
      CRCbuf = calcByte(pData[i], CRCbuf);
    }
    bt1 = pData[len-1];
    bt2 = pData[len-2];
    crccheck = CRCbuf;
    if(type == AT_CRC)
      return (CRCbuf==((bt1<<8)|(bt2)));
    else
      return (CRCbuf==((bt2<<8)|(bt1)));
  }
};

uint16_t CRC_calcByte (uint8_t Sbyte, uint16_t CRCbuf)
{
  return CRC_global.calcByte(Sbyte, CRCbuf);
}

uint16_t CRC_calc(uint8_t volatile * pData, uint32_t len)
{
  return CRC_global.calc(pData, len, AT_CRC);
}

uint32_t CRC_check(const uint8_t * pData, uint32_t len)
{
  if(CRC_global.check(pData, len, AT_CRC))
    return 1;
  else
    return 0;
}